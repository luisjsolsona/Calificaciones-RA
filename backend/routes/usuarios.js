...
router.get('/', auth(['admin', 'docente']), (req, res) => {
  const { rol } = req.query;
  const rows = rol
    ? db.prepare('SELECT id,email,usuario,nombre,rol,alumno_nombre,created_at FROM usuarios WHERE rol=? ORDER BY nombre').all(rol)
    : db.prepare('SELECT id,email,usuario,nombre,rol,alumno_nombre,created_at FROM usuarios ORDER BY nombre').all();
  res.json(rows);
});
...
router.put('/:id', auth(['admin']), (req, res) => {
  const { nombre, email, usuario, rol, alumno_nombre } = req.body;
  db.prepare('UPDATE usuarios SET nombre=?,email=?,usuario=?,rol=?,alumno_nombre=? WHERE id=?')
    .run(nombre, email, usuario || null, rol, alumno_nombre || null, req.params.id);
  res.json({ ok: true });
});
