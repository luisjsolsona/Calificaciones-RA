...
function NotebookIcon({ size = 36 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 40 50" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ flexShrink:0 }}>
      ...
      <text x="24" y="43" textAnchor="middle" fill="white" fontSize="13" fontWeight="900" fontFamily="'DM Sans',sans-serif" letterSpacing="-0.5">RA</text>
    </svg>
  );
}
...
function ImportPanel({ placeholder, onImport, onClose }) {
  ...
  return (
    <div style={{ background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:10, padding:14, display:"flex", flexDirection:"column", gap:10 }}>
      ...
      <textarea
        value={text}
        onChange={e => { setText(e.target.value); setJustPasted(false); }}
        onPaste={handlePaste}
        placeholder={placeholder}
        rows={6}
        style={{ ...IS, fontFamily:"'Fira Mono',monospace", fontSize:12, resize:"vertical", lineHeight:1.6 }}
      />
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
        <button onClick={apply} disabled={!text.trim()} style={{ ...BS, opacity: text.trim() ? 1 : 0.5 }}>
          ↑ Importar
        </button>
...
function tipoBadge(tipo) {
  ...
  return (
    <span style={{
      ...
      background:esExam?"#f5f3ff":"#ecfeff",
      color:esExam?"#7c3aed":"#0891b2",
      border:`1px solid ${esExam?"#ddd6fe":"#a5f3fc"}`,
...
export default function CuadernoCalificaciones() {
  ...
  // estados panel import por entidad
  ...
  const savedMsgTimer = useRef(null);
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      try {
        ...
        setShowSaved(true);
        savedMsgTimer.current = setTimeout(() => setShowSaved(false), 2500);
      } catch(_) {}
    }, 800);
  ...
  // ── Exportar todo ─────────────────────────────────────────────────────────
  function exportTodo() {
    downloadJSON({ alumnos, ras, uds, actividades }, "cuaderno-ra");
  }
  function importTodo(e) {
    ...
    reader.onload = ev => {
      try {
        ...
        if (d.uds)         setUDs(d.uds);
        if (d.actividades) setActividades(d.actividades);
      } catch(_) { alert("Error al importar: archivo inválido"); }
    };
    reader.readAsText(file);
    e.target.value = "";
  }
  ...
  function handleImportAlumnos(text, mode) {
    ...
    if (trimmed.startsWith("[")) {
      try { items = JSON.parse(trimmed).filter(x=>x.nombre).map(x=>({ nombre: String(x.nombre).trim() })); }
    ...
    if (mode === "replace") {
      ...
      setAlumnos(prev => {
        const existentes = new Set(prev.map(a=>a.nombre));
        const maxId = prev.length ? Math.max(...prev.map(a=>a.id)) : 0;
        const nuevos = items.filter(x=>!existentes.has(x.nombre));
        return [...prev, ...nuevos.map((x,i) => ({ id:maxId+i+1, nombre:x.nombre }))];
      });
    }
  }
  ...
  function handleImportRAs(text, mode) {
    try {
      ...
      const newRAs = parsed.map(r => ({
        id: r.id || "RA_"+Date.now(),
        titulo: r.titulo || "",
        descripcion: r.descripcion || "",
        peso: r.peso ?? null,
        pctAct: r.pctAct ?? 40,
        pctExam: r.pctExam ?? 60,
      }));
      if (mode === "replace") {
        setRAs(newRAs);
      } else {
        setRAs(prev => {
          ...
          return [...prev, ...newRAs.filter(r=>!ids.has(r.id))];
        });
      }
    } catch(_) { alert('JSON inválido. Formato: [{"id":"RA1","titulo":"...","pctAct":40,"pctExam":60}]'); }
  }

  function handleImportUDs(text, mode) {
    try {
      const parsed = JSON.parse(text.trim());
      const newUDs = parsed.map(u => ({
        id: u.id || "UD_"+Date.now(),
        titulo: u.titulo || "",
        descripcion: u.descripcion || "",
        ras: Array.isArray(u.ras) ? u.ras : [],
      }));
      if (mode === "replace") {
        setUDs(newUDs);
      } else {
        setUDs(prev => {
          ...
          return [...prev, ...newUDs.filter(u=>!ids.has(u.id))];
        });
      }
    } catch(_) { alert('JSON inválido. Formato: [{"id":"UD1","titulo":"...","ras":["RA1"]}]'); }
  }

  function handleImportActividades(text, mode) {
    try {
      const parsed = JSON.parse(text.trim());
      const newActs = parsed.map(a => {
        const notas = { ...(a.notas || {}) };
        alumnos.forEach(al => { if (notas[al.id] === undefined) notas[al.id] = ""; });
        return {
      ...
      if (mode === "replace") {
        ...
        setActividades(prev => {
          ...
          return [...prev, ...newActs.filter(a=>!ids.has(a.id))];
        });
      }
    } catch(_) { alert('JSON inválido. Formato: [{"nombre":"...","tipo":"actividad","ras":["RA1"],"ud":"UD1","peso":50}]'); }
  }
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16 }}>
          {[
            ...
            <div key={c.label} style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:16, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
          <div style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:16, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:16, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={raData} barSize={32}>
              ...
              <Tooltip contentStyle={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:8, color:"#0f172a" }} formatter={v=>v.toFixed(2)}/>
              <Bar dataKey="media" radius={[4,4,0,0]} fill="#4f46e5"/>
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div style={{ borderRadius:12, overflow:"hidden", border:"1px solid #e2e8f0", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {alumnos.map((al,i)=>(
                <tr key={al.id} style={{ background:i%2===0?"#ffffff":"#f8fafc", cursor:"pointer" }}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ borderRadius:12, overflow:"hidden", border:"1px solid #e2e8f0", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {alumnos.map((al,i)=>(
                <tr key={al.id} style={{ background:i%2===0?"#ffffff":"#f8fafc" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
          <p style={{ color:"#64748b", fontSize:13, margin:0 }}>
            Peso vacío → distribución proporcional automática.
          </p>
          <div style={{ display:"flex", gap:8 }}>
        ...
        {showImportRAs && (
          <ImportPanel
            placeholder={'Pega JSON:\n[\n  {"id":"RA1","titulo":"...","descripcion":"...","peso":null,"pctAct":40,"pctExam":60},\n  ...\n]'}
            onImport={handleImportRAs}
            onClose={()=>setShowImportRAs(false)}
          />
        )}
        {ras.map(ra=>(
          <div key={ra.id} style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:16, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
            ...
            <div style={{ display:"flex", gap:14, alignItems:"center", flexWrap:"wrap" }}>
              ...
              {(ra.pctAct??40)+(ra.pctExam??60)!==100 && (
                <span style={{ color:"#dc2626", fontSize:11 }}>⚠ Suma {(ra.pctAct??40)+(ra.pctExam??60)}%</span>
              )}
            </div>
          </div>
        ))}
  ...
  function TabUDs() {
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {showImportUDs && (
          <ImportPanel
            placeholder={'Pega JSON:\n[\n  {"id":"UD1","titulo":"Unidad 1","descripcion":"...","ras":["RA1"]},\n  ...\n]'}
        ...
        <div style={{ borderRadius:12, overflow:"hidden", border:"1px solid #e2e8f0", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              {uds.map((ud,i)=>(
                <tr key={ud.id} style={{ background:i%2===0?"#ffffff":"#f8fafc" }}>
                  <td style={TD}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {uds.map(ud=>(
            <div key={ud.id} style={{ display:"flex", gap:10, alignItems:"center", background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:10, padding:"10px 12px", boxShadow:"0 1px 2px rgba(0,0,0,0.04)" }}>
  ...
  function TabActividades() {
    ...
    function commitDrop(targetRaId, targetTipo, targetIdx) {
      ...
      setActividades(prev=>{
        ...
        return prev.map(a=>
          a.id===draggingId ? {...a,ras:[targetRaId],tipo:targetTipo,orden:map[a.id]??0}
          : map[a.id]!==undefined ? {...a,orden:map[a.id]} : a
        );
      });
    ...
    function ActRow({ act, raId, tipo, idx }) {
      ...
      const incompleta = actividadIncompleta(act);
      const problemas  = actividadProblemas(act);
      ...
      return (
        <div
          ...
          style={{
            ...
            background: isOver?"#eff6ff":incompleta?"#fef2f2":"#fafafa",
            border:`1px solid ${isOver?"#4f46e5":incompleta?"#fca5a5":"#e2e8f0"}`,
          ...
          {isEdit ? (
            ...
            <>
              <span style={{ flex:2, color:"#0f172a", fontSize:13 }}>{act.nombre}</span>
              {incompleta && (
                <span title={`Incompleto: ${problemas.join(", ")}`}
                  style={{ color:"#dc2626", fontSize:11, flexShrink:0, background:"#fee2e2", border:"1px solid #fca5a5", borderRadius:4, padding:"1px 6px" }}>
                  ⚠ {problemas.join(" · ")}
                </span>
              )}
              ...
              <button onClick={()=>setEditingAct(act.id)}
                style={{ background:"none", border:"1px solid #e2e8f0", borderRadius:6, color:"#64748b", cursor:"pointer", padding:"3px 8px", fontSize:12 }}>✏</button>
      ...
      return (
        <div
          ...
          style={{
            ...
            color:isOver?"#4f46e5":"#94a3b8", fontSize:12,
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
        {/* Alerta global si hay actividades incompletas */}
        {actsFaltantes>0 && (
          <div style={{ background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:10, padding:"10px 16px", display:"flex", alignItems:"center", gap:10 }}>
            ...
            <span style={{ color:"#991b1b", fontSize:13, fontWeight:500 }}>
              {actsFaltantes} actividad{actsFaltantes>1?"es":""} con campos sin completar (RA, UD o peso).
              Corrígelas para que el cálculo sea correcto.
            </span>
          </div>
        )}
        ...
        {showImportActs && (
          <ImportPanel
            placeholder={'Pega JSON:\n[\n  {"nombre":"Práctica","tipo":"actividad","ras":["RA1"],"ud":"UD1","peso":50},\n  {"nombre":"Examen","tipo":"examen","ras":["RA1"],"ud":"UD1","peso":50},\n  ...\n]'}
        ...
        <div style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, padding:16, display:"flex", flexDirection:"column", gap:10, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
            <span style={{ color:"#64748b", fontSize:12 }}>RAs vinculados:</span>
            {ras.map(ra=>(
        ...
        {ras.map(ra=>{
          ...
          return (
            <div key={ra.id} style={{ background:"#ffffff", border:"1px solid #e2e8f0", borderRadius:12, overflow:"hidden", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr" }}>
                <div style={{ borderRight:"1px solid #f1f5f9", padding:"12px 12px" }}>
        ...
        {/* Tabla de notas con destacado rojo */}
        <div style={{ borderRadius:12, overflow:"auto", border:"1px solid #e2e8f0", maxHeight:400, boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
            ...
            <tbody>
              {ras.map(ra=>{
                ...
                return grupo.map((act,i)=>{
                  ...
                  return (
                    <tr key={act.id} style={{ background:i%2===0?"#ffffff":"#f8fafc", borderLeft:inc?"3px solid #fca5a5":"3px solid transparent" }}>
                      <td style={TD}>
                        {inc && <span title={actividadProblemas(act).join(", ")} style={{ color:"#dc2626", marginRight:5, fontSize:13 }}>⚠</span>}
                        {act.nombre}
                      </td>
                          ...
                          <td key={al.id} style={{ ...TD, textAlign:"center", cursor:"pointer" }} onClick={()=>setEditingNota(k)}>
                            {editingNota===k
                              ? <input autoFocus type="number" min={0} max={10} step={0.1} defaultValue={nota}
                                  ...
                                  style={{ width:52, textAlign:"center", background:"#ffffff", border:"1px solid #4f46e5", borderRadius:4, color:"#0f172a", padding:"2px 4px", fontSize:13 }}/>
                              : notaBadge(nota)}
                          </td>
                        );
                      })}
        ...
        <p style={{ color:"#94a3b8", fontSize:12, margin:0 }}>💡 Arrastra ⠿ para reordenar · ✏ para editar · Clic en nota para modificar · ⚠ campo sin rellenar</p>
      </div>
    );
  }
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        {ras.map(ra=>{
          ...
          return (
            <div key={ra.id} style={{ borderRadius:12, overflow:"hidden", border:"1px solid #e2e8f0", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f1f5f9", color:"#0f172a", fontFamily:"'DM Sans','Segoe UI',sans-serif", paddingBottom:48 }}>
      <div style={{ background:"linear-gradient(135deg,#eef2ff 0%,#e0f2fe 100%)", borderBottom:"1px solid #e2e8f0", padding:"20px 28px 0", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
        <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, paddingBottom:4 }}>
            {showSaved && <span style={{ color:"#059669", fontSize:12, fontWeight:600 }}>✓ Guardado</span>}
