...
export default function CuadernoCalificaciones() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (data.ras)         setRAs(data.ras);
        if (data.uds)         setUDs(data.uds);
  ...
  function exportData() {
    ...
    a.href     = url;
    a.download = `cuaderno-ra-${new Date().toISOString().slice(0,10)}.json`;
    a.click();
      ...
      return (
        <div
          ...
          style={{
            ...
            border: `1px solid ${isOver ? "#4f46e5" : "#e2e8f0"}`,
      ...
      <div style={{ background:"linear-gradient(135deg,#eef2ff 0%,#e0f2fe 100%)", borderBottom:"1px solid #e2e8f0", padding:"20px 28px 0", boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
        <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, paddingBottom:4 }}>
            {showSaved && (
              <span style={{ color:"#059669", fontSize:12, fontWeight:600, display:"flex", alignItems:"center", gap:4 }}>
                ✓ Guardado
              </span>
            )}
            <span style={{ color:"#94a3b8", fontSize:12 }}>{alumnos.length} alumnos · {actividades.length} actividades</span>
            <button onClick={exportData} style={{ ...BS, background:"#ffffff", color:"#4f46e5", border:"1px solid #c7d2fe", padding:"6px 12px", fontSize:12 }}>
              ↓ Exportar
            </button>
            <label style={{ ...BS, background:"#ffffff", color:"#64748b", border:"1px solid #e2e8f0", padding:"6px 12px", fontSize:12, cursor:"pointer" }}>
              ↑ Importar
